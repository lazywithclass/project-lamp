...
To compile editable PureScript code, click into the snippet you want executed and press `CTRL + ENTER`, or on OS X press `CMD + ENTER`, you will not see anything if the compilation goes well, while you will see an error if something went wrong: try it out by removing the last `)` in the above editable code.
...
Again, if you don't see any errors chances are you added the correct types.
